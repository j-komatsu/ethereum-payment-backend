...
**今回の対応:** 未実装のまま
**妥協した理由:** 初期構成フェーズであり、認証の設計方針をまだ決めていない。
**⚠️ 重要：** これは軽微な技術的負債ではなく**本番稼働前に必ず対応が必要な項目**。未対応のまま本番に出ると、誰でも決済注文の作成・照会が可能になる。
**補足：** 当初 `health show-details: when-authorized` を設定したが、Spring Security 未導入の場合「認証済みユーザーが存在しないため詳細が返らない」という動作になり、結果的に `never` と同じ挙動になった。これを受けて `never` に明示変更済み。
**今後の対応時期:** Phase 5〜6（必須）
...
- Phase 5〜6：Spring Security によるAPI認証の実装（**本番前必須**）
- `ddl-auto: validate` で起動するには事前にスキーマが存在している必要がある。Flyway / Liquibase が推奨手段だが必須ではない（手動DDLでも動作する）→ Phase 7 で整備予定
