...
| 理由 | 説明 |
|---|---|
| **ストレージコスト回避** | コントラクトのストレージ（storage）に書き込むと多くのガスを消費するが、Log への書き込みはストレージの約 1/8 のコストで済む |
...
| 方式 | 制限事項 |
|---|---|
| HTTP ポーリング | 1日あたり 3,000,000 リクエストまで（Polygon は消費が速い） |
...
Block N (A)  ← 後でこちらが採用される
         ↘
          Block N+1 ← こちらが孤立ブロック（uncle）になる
Block N (B)
```
...
取引所がPolygo nで256ブロック確認を要求するのは、過去に数十〜百数十ブロックに及ぶ大規模 reorg が発生した事例があったためである。本 PoC では学習・開発用途として20ブロック（約40秒）を採用している。
...
**PoS 移行後（Merge 後）：**
- 約6.4分（2エポック = 64スロット）ごとに **チェックポイント** が設けられる
...
| ブロック時間 | 12秒 | 約2秒 |
| 20確認での安全性 | 約4分（十分） | 約40秒（実用的な安全水準） |
