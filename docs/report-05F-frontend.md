...
| **EIP-2612 Permit** | オフチェーン署名で承認 → ガス不要 | Approve のガスレス版 |
| **Streaming（Sablier）** | 毎秒単位で自動送金 | サブスク・時間課金向け |
| **Meta-transaction** | リレーヤーがガス代を肩代わり | ユーザーが ETH 不要 |
| **Account Abstraction** | スマートウォレットがガスをトークンで払う | EIP-4337。最新の解決策 |
...
Phase 5       【即時決済 / Pull型 + ガスレス】
              Permit（EIP-2612）でオフチェーン署名
              → バックエンドが transferFrom で引き落とし
...
|---|---|---|---|---|
| Direct Transfer（手動） | 悪い（毎回ウォレット操作） | ユーザー | 高 | 低 |
| 入金検知（Phase 4） | 普通（送金後に確認待ち） | ユーザー | 高 | 中 |
| Permit + transferFrom（Phase 5） | 良い（署名1回で完了） | バックエンド | 中 | 高 |
| Streaming（Phase 6） | 最良（自動） | バックエンド | 中 | 高 |
| Account Abstraction | 最良（ガス不要） | スポンサー | 高 | 非常に高 |
