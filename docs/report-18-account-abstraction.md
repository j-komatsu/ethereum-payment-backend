...
2023年3月に EntryPoint コントラクトが Ethereum メインネットにデプロイされた仕様（プロトコルレベルの変更不要で実現できる点が最大の特徴）。
...
```
確立済み（〜2025年）:
  ✅ EIP-4337 メインネットデプロイ済み（2023年3月）
  ✅ Biconomy, ZeroDev, Pimlico などのインフラ整備完了
  ✅ Base (Coinbase L2) でのガスレス体験が普及中
  ✅ EIP-7702（ネイティブ AA）: 2025年5月 Pectra アップグレードで本番稼働済み

現在〜今後（2026年〜）:
  🎯 スマートフォンのセキュアエンクレーブ = ウォレット（普及フェーズ）
  🎯 「ウォレット」という概念が消える → 普通のアプリと同じ UX
  ⚠️ まだ EOA が主流だが SCW への移行が加速中
```

**EIP-7702（2025年5月 Pectra アップグレードで導入済み）:**
