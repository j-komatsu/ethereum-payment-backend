...
Phase 4  ✅ 完了   Transfer イベント監視 + 入金検知
Phase 5  ✅ 完了   ウォレット認証 + スマコン事前承認決済（SIWE・Permit）
Phase 5-F ✅ 完了  フロントエンド（Next.js 15 + Wagmi v2 + RainbowKit）送金UI
Phase 6  🔜 次     自動購入・サブスク + テスト + OpenAPI
...
### Day 9 — SpringDoc OpenAPI 導入（依存追加 + 表示確認のみ）

> Day 9 では依存追加と Swagger UI の表示確認のみ行う。
> @Operation/@Tag/@Schema の全エンドポイント対応は Day 29（Phase 6）で実施する。
...
JPYC は Polygon mainnet のコントラクトのみ存在する。Polygon Amoy testnet に公式テストコントラクトはない。
以下のいずれかを選択して `.env.example` と README に記載すること。

| 選択肢 | 内容 | 推奨度 |
|---|---|---|
| **A: Hardhat --fork** | `npx hardhat node --fork ${WEB3J_POLYGON_ENDPOINT}` でローカルフォーク | ⭐ 推奨 |
| B: Polygon Amoy + 自前ERC-20 | 自分でサンプルERC-20をデプロイしてテスト | 次点 |
| C: mainnet 直接 | 少額の実JPYCでテスト | リスクあり |

**推奨: 選択肢A（Hardhat mainnet fork）**
- mainnet の JPYC コントラクトをそのまま使える
- 実際のイベント・ブロックで動作確認できる
- ガス代・実 JPYC のリスクなし

```bash
# Hardhat fork 起動コマンド（application-dev.yml で localhost:8545 を向ける）
npx hardhat node --fork ${WEB3J_POLYGON_ENDPOINT}
```

**【必須】PollerState エンティティ設計**

最後に処理したブロック番号を保存するテーブルを Day 16 着手前に確定する。

| カラム | 型 | 説明 |
|---|---|---|
| `token` | VARCHAR(10) PK | トークン種別（JPYC 等）|
| `last_processed_block` | BIGINT | 最後に処理したブロック番号 |
| `updated_at` | TIMESTAMP | 更新日時 |

**【必須】Confirmations 数の設計判断**

Polygon では 20〜30 ブロック確認後に `CONFIRMED` にするのが一般的。
`REQUIRED_CONFIRMATIONS = 20` をアプリケーション定数として定義する。
...
**Day 17 実装と同時に追加（テストを後回しにしない）:**

```
PaymentServiceTest#confirmPayment — Mockito 単体テスト
  - 正常系: 一致金額 → CONFIRMED
  - 正常系: 超過金額 → OVERPAID
  - 正常系: 不足金額 → UNDERPAID
  - 異常系: 対象注文なし（スキップ）
  - 異常系: txHash 重複（べき等チェック）
```

**コミットメッセージ例:**
```
feat: Transfer入金検知→PaymentOrder自動確認（CONFIRMED/OVERPAID/UNDERPAID）
test: PaymentService#confirmPayment 単体テスト追加
```
...
**【必須】Spring Security 追加後の既存エンドポイント公開方針**

Day 21 で Spring Security を追加すると、既存の全エンドポイントが突然 401 になる。
着手前に公開/認証方針を確定しておくこと。

| エンドポイント | 方針 | 理由 |
|---|---|---|
| `/api/v1/auth/**` | PERMIT_ALL | 認証前なので公開必須 |
| `/api/v1/chain/**` | PERMIT_ALL（開発中）/ AUTHENTICATED（本番） | 残高照会は学習用で公開でも可 |
| `/api/v1/payments/**` | AUTHENTICATED | 決済注文は認証必須 |
| `/actuator/health` | PERMIT_ALL | ヘルスチェックは公開 |

**【必須】spender ウォレット準備**

Phase 5 Day 23（transferFrom 実装）の前に spender ウォレットを用意する。

```
1. Web3j/MetaMask で専用ウォレット作成
2. Hardhat fork 環境では impersonation でガス調達可能
3. .env.example に SPENDER_PRIVATE_KEY キー名を追加（値は絶対コミットしない）
4. POL（Polygon ガス代通貨）を用意（mainnet では少額の実POLが必要）
```

**【必須】Day 21 でのテスト追加**

```
SiweServiceTest — 署名検証の正常系・異常系
  - 正常系: 正しい署名 → アドレス復元成功
  - 異常系: nonce 使用済み → 拒否
  - 異常系: domain 不一致 → 拒否
  - 異常系: 有効期限切れ → 拒否
...
- 既存エンドポイントの公開/認証方針を SecurityConfig に反映する（上記「Phase 5 着手前」参照）
...
### Day 22-23 — EIP-2612 Permit + approve/transferFrom 決済（2日構成）

> ⚠️ PM/PMOレビュー指摘: EIP-712 ハッシュ計算（domain_separator + structHash）は
> Web3j との格闘が伴い、1日では完結しないリスクがある。Day 22-23 を一体で設計する。

**Day 22 目標:** EIP-712 署名データ生成とオンチェーン permit TX の送信まで
...
### Day 23 — approve/transferFrom 即時決済（Day 22 の続き）
...
> ⚠️ **開始タイミング:** Phase 5 バックエンド（Day 25）完了後に開始する。
> Day 21F〜25F の「F日番号」は Phase 5 完了後の相対日数。
> SIWE バックエンドが動いていない状態でフロントの SIWE 連携は実装できないため、直列構成とする。
