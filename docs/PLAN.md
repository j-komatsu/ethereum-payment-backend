...
```
認証フロー:
1. GET /api/v1/auth/nonce → ランダムなワンタイムトークンを返す
2. フロント側：HashPort Wallet でメッセージに署名（EIP-4361形式）
3. POST /api/v1/auth/verify → 署名を検証してウォレットアドレスを確認
4. セッション or JWT を発行して以降のAPI認証に使う
```
...
**設計判断のポイント:**
- バックエンドが送金操作をするため、サーバー秘密鍵の管理が必要
...
**新規ファイル:** `src/main/java/com/example/payment/chain/StreamingPaymentService.java`
...
2. @Scheduled で毎月1日に transferFrom を実行
...
**コミットメッセージ例:**
```
feat: JPYC自動購入ジョブ（allowance+Scheduledで定期引き落とし）
```
